...
public class ProcessArgs {
	...
	private static void applyArgument(Object config, StateMap<ArgumentDefinition, Integer> argcount, ArgumentDefinition ad,	String val) {
		Class<?> clz = Reflection.fieldType(config, ad.toVar);
		if (Collection.class.isAssignableFrom(clz)) {
			for (String s : val.split(ad.splitChar))
				if (s != null && s.length() > 0) {
					Object obj = s;
					if (ad.recordArgument)
						obj = new Argument(ad.text, s);
					Reflection.setField(config, ad.toVar, obj); // append to collection
				}
		} else
			Reflection.setField(config, ad.toVar, val);
		argcount.op(ad, 1, new FuncR1<Integer, Integer>() {
